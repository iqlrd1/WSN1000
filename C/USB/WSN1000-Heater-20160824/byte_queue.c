...
static void copyIntoBuffer(const uint8 *p_data, uint16 len)
{
    ...
    if (len > BUFFER_LEN)
    {
        ...
        p_data += len - BUFFER_LEN;//以缓冲区大小为准
    ...
    if (g_tail + len >= BUFFER_SIZE)
    {
        ...
        MemCopy(&g_queue[g_tail], p_data, available);//缓冲→队列
        
        /* Update g_tail */
        g_tail = len - available;//剩余的待复制到缓冲区的大小
...
static uint16 peekBuffer(uint8 *p_data, uint16 len)
{
    ...
    {
        /* Calculate how much space there is till the end of the buffer */
        const uint16 available = BUFFER_SIZE - g_head;//能查看的最大空间
        
        /* Copy data up to end of buffer */
        MemCopy(p_data, &g_queue[g_head], available);//队列→缓冲区
        
        /* Update g_peek */
        g_peek = peeked - available;//剩余还要查看的大小即时更新
...
uint16 BQPopBytes(uint8 *p_data, uint16 len)
{
    ...
    uint16 peeked = peekBuffer(p_data, len);//store to buffer from queue
    
    /* Remove the peeked data from the queue */
    BQCommitLastPeek();//update queue pointer g_head=g_peek
