...
static void gattAdvertTimerHandler(timer_id tid)
{
    if (g_tsapp_data.gatt_data.app_tid == tid)
    {
...
extern void GattTriggerFastAdverts(void)
{
    ...
    MemCopy(&adv_params.bd_addr.addr, &g_tsapp_data.random_bd_addr, 
                                                            sizeof(BD_ADDR_T));
    ...
    g_tsapp_data.gatt_data.advert_timer_value = 
                                            ADVERT_INTERVAL + appRandomDelay();

    /* If GATT bearer is enabled send connectable advert */
    if (g_tsapp_data.bearer_data.bearerEnabled & BLE_GATT_SERVER_BEARER_MASK)
    {
        /* Set advertisement parameters */
        ...
        g_tsapp_data.gatt_data.app_tid = TimerCreate(
                                      g_tsapp_data.gatt_data.advert_timer_value,
